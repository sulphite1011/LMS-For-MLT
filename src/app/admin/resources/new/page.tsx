...
export default function NewResourcePage() {
  ...
  const onFileDrop = useCallback((acceptedFiles: File[]) => {
    ...
    if (acceptedFiles.some(f => f.size > 10 * 1024 * 1024)) {
  ...
  const addExternalLink = () => {
    setExternalLinks(prev => [...prev, { label: "", url: "", id: `el-${Date.now()}` }]);
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-2xl p-6 shadow-sm space-y-4">
          ...
          <div>
            ...
            <input type="text" value={title} onChange={e => setTitle(e.target.value)} placeholder="Enter resource title" className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:border-[#14b8a6] focus:ring-2 focus:ring-[#14b8a6]/20 focus:outline-none text-sm" required suppressHydrationWarning />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Subject *</label>
              <input type="text" value={subjectName} onChange={e => setSubjectName(e.target.value)} placeholder="e.g. Hematology, Microbiology..." className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:border-[#14b8a6] focus:ring-2 focus:ring-[#14b8a6]/20 focus:outline-none text-sm" required suppressHydrationWarning />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Type *</label>
              <select value={resourceType} onChange={e => setResourceType(e.target.value)} className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:border-[#14b8a6] focus:ring-2 focus:ring-[#14b8a6]/20 focus:outline-none text-sm bg-white" required suppressHydrationWarning>
          ...
          <div>
            ...
            <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Brief description of this resource..." rows={3} className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:border-[#14b8a6] focus:ring-2 focus:ring-[#14b8a6]/20 focus:outline-none text-sm resize-none" />
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white rounded-2xl p-6 shadow-sm space-y-4">
          ...
          <div {...getBannerRootProps()} className="border-2 border-dashed border-gray-200 hover:border-[#14b8a6] rounded-xl p-6 text-center cursor-pointer transition-colors">
            <input {...getBannerInputProps()} />
            {bannerPreview ? (
              <div className="relative">
                <img src={bannerPreview} alt="Banner preview" className="w-full h-40 object-cover rounded-lg" />
                <button type="button" onClick={e => { e.stopPropagation(); setBannerFile(null); setBannerPreview(null); }} className="absolute top-2 right-2 p-1 bg-red-500 text-white rounded-full">
                  <X className="w-4 h-4" />
                </button>
              </div>
            ) : (
          ...
          <div>
            ...
            <input type="url" value={bannerImageUrl} onChange={e => setBannerImageUrl(e.target.value)} placeholder="https://example.com/image.jpg" className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:border-[#14b8a6] focus:ring-2 focus:ring-[#14b8a6]/20 focus:outline-none text-sm" />
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-white rounded-2xl p-6 shadow-sm space-y-4">
          <h2 className="font-semibold text-[#1e293b] text-lg flex items-center gap-2">
            <FileText className="w-5 h-5 text-[#14b8a6]" /> PDF Files
            <span className="text-xs text-gray-400 font-normal ml-1">(up to 5 files, max 10MB each)</span>
          </h2>
          <div {...getFileRootProps()} className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all ${isFileDragActive ? "border-[#14b8a6] bg-teal-50/50" : "border-gray-200 hover:border-[#14b8a6]"}`}>
            ...
            <p className="text-sm text-gray-500">{isFileDragActive ? "Drop files here..." : "Drag & drop PDFs here, or click to browse"}</p>
            ...
            <div className="space-y-2">
              {pdfFiles.map(({ file, id }) => (
                <div key={id} className="flex items-center gap-3 bg-slate-50 rounded-xl px-4 py-2.5">
                  ...
                  <button type="button" onClick={() => setPdfFiles(prev => prev.filter(f => f.id !== id))} className="p-1 hover:bg-red-50 rounded-full">
          ...
          {saving && uploadProgress > 0 && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-2">
              ...
              <div className="w-full bg-gray-200 rounded-full h-2">
                <motion.div className="bg-[#14b8a6] h-2 rounded-full" initial={{ width: 0 }} animate={{ width: `${uploadProgress}%` }} transition={{ duration: 0.3 }} />
              </div>
            </motion.div>
          )}
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="bg-white rounded-2xl p-6 shadow-sm space-y-4">
          <h2 className="font-semibold text-[#1e293b] text-lg flex items-center gap-2">
            <Globe className="w-5 h-5 text-[#14b8a6]" /> External Links
            <span className="text-xs text-gray-400 font-normal ml-1">(Google Drive, Slides, GitHub, etc.)</span>
          </h2>
          {externalLinks.map(link => (
            <div key={link.id} className="flex gap-3 items-start">
              <div className="flex-1 grid grid-cols-1 sm:grid-cols-3 gap-2">
                <input type="text" value={link.label} onChange={e => updateExternalLink(link.id, "label", e.target.value)} placeholder="Label (e.g. Study Notes)" className="px-4 py-2.5 border border-gray-200 rounded-xl focus:border-[#14b8a6] focus:ring-2 focus:ring-[#14b8a6]/20 focus:outline-none text-sm" />
                <input type="url" value={link.url} onChange={e => updateExternalLink(link.id, "url", e.target.value)} placeholder="https://drive.google.com/..." className="sm:col-span-2 px-4 py-2.5 border border-gray-200 rounded-xl focus:border-[#14b8a6] focus:ring-2 focus:ring-[#14b8a6]/20 focus:outline-none text-sm" />
              </div>
              {externalLinks.length > 1 && (
                <button type="button" onClick={() => removeExternalLink(link.id)} className="p-2 hover:bg-red-50 rounded-lg mt-0.5">
                  <X className="w-4 h-4 text-red-400" />
                </button>
              )}
            </div>
          ))}
          <button type="button" onClick={addExternalLink} className="flex items-center gap-2 text-sm text-[#14b8a6] hover:text-[#0d9488] font-medium">
          ...
          {youtubeUrls.map((url, i) => (
            <div key={i} className="flex gap-3 items-start">
              <div className="flex-1">
                <input type="url" value={url} onChange={e => { const updated = [...youtubeUrls]; updated[i] = e.target.value; setYoutubeUrls(updated); }} placeholder="https://www.youtube.com/watch?v=..." className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:border-[#14b8a6] focus:ring-2 focus:ring-[#14b8a6]/20 focus:outline-none text-sm" />
